...
def separate_train_test(fig, fig_c, ratio=0, select=[]):
    ...
    if ratio != 0:
        ...
        figc = np.array(figc_new)
    
    fig_t = np.array(fig_t)
    fig_tc = np.array(fig_tc)
    return fig, figc, fig_t, fig_tc
...
def get_effective_data(sep_indices, ratio, char_c1, char_c7):
    '''
       ...
       ratio - a number, the ratio of the number of the class having the least training objects will be set as the benchmark.
       char_c1 - a list, the characteristic objects of the first class.
       char_c7 - a list, the characteristic objects of the last class.
    '''

    num = len(sep_indices[0])
...
def find_interested_area(fig, figc):
    ...
    cr_xe = figc.shape[1]
    cr_ye = figc.shape[2]
    for i in range(len(figc)):
        interest_indxt, interest_indexyt = np.where(figc[i] != 1)
        interest_indxb, interest_indexyb = np.where(figc[i] != 7)
    ...
    cr_xe = min(figc.shape[1], cr_xe + 50)
    return cr_xs, cr_xe
...
def data_augmentation(patch, minb, maxb, p_transform):
    ...
    if len(patch.shape) == 3:           
        for i in range(patch.shape[0]):          
            ...
            img = (p_transform(patch[i])[0]).float()
...
def dynamic_data_augmentation(patch, minb, maxb, d_transform, prob):
    ...
    if p < prob:
        ...
        if len(patch.shape) == 3:
            for i in range(patch.shape[0]):  
                patch[i] = ((255.*(patch[i]-minb[i]))/(maxb[i]-minb[i])).int()
                img = (d_transform(patch[i])[0]).float()
